...
def insert_data(name, phone, email, password, address, apartment_number):
    query = """
    INSERT INTO customers (name, phone, email, password, address, apartment_number)
    VALUES (%s, %s, %s, %s, %s, %s)
    """
    cur.execute(
        query, 
...
with st.form("signup"):
    ...
    if submit:
        if name and email and password and address:
            ...
            insert_data(name, phone, email, password, address, apt)
